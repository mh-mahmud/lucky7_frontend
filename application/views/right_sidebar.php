							...
							<div class="banCont latent">
								<div class="right-banners-block" style="display: block;" id="upcoming_score_ajax">
									<?php
										...
										if (!empty($up_match)):
										foreach ($up_match as $vmatch) :
										$team1_coin = $this->db->query("SELECT option_coin FROM match_option_details WHERE match_option_id='{$vmatch->option_id}' AND option_serial = '1'")->row();
										
										//print_r($vmatch);
										if(!empty($team1_coin)) {
											$team1_coin = $team1_coin->option_coin;
										}
										else {
											$team1_coin = 1;
										}
										
										$team2_coin = $this->db->query("SELECT option_coin FROM match_option_details WHERE match_option_id='{$vmatch->option_id}' AND option_serial = '3'")->row();
										
										if(!empty($team2_coin)) {
											$team2_coin = $team2_coin->option_coin;
										}
										else {
											$team2_coin = 1;
										}
