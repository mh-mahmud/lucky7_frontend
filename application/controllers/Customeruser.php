...
class Customeruser extends CI_Controller
{
	...
	public function view_profile()
	{
		...
		$data['coin'] = 0;
		$user_coin = $this->db->query("SELECT * FROM `my_coin` WHERE user_id={$this->user_id} ORDER BY id DESC LIMIT 1")->row();
		if( !empty($user_coin) ) {
			$data['coin'] = $user_coin->current_balance;
		}
	...
	public function coin_transfer()
	{
		...
		// disabled coin transfer
		//$this->session->set_flashdata('error', 'Coin transfer disabled');
	...
	{
		$data = [];
		$data['acc_data'] = $this->db->query("SELECT * FROM withdraw_account")->result();
